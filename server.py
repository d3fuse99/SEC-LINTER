...
class SQLiteStorage:
    def __init__(self):
        ...
        cursor.execute('CREATE TABLE IF NOT EXISTS limits (ip TEXT, timestamp REAL)')
    ...
    def is_limited(self, ip_address):
        ...
        try:
            conn = sqlite3.connect(self.db_path, timeout=5.0)
            cursor = conn.cursor()
            cursor.execute('DELETE FROM limits WHERE ? - timestamp > 60', (now,))
            cursor.execute('SELECT COUNT(*) FROM limits WHERE ip = ?', (ip_address,))
            count = cursor.fetchone()[0]
            if count >= 60:
                conn.commit()
                conn.close()
                return True
            cursor.execute('INSERT INTO limits VALUES (?, ?)', (ip_address, now))
            conn.commit()
            conn.close()
        except Exception:
            pass
...
def init_db():
    global limiter_storage
    limiter_storage = SQLiteStorage()

def is_rate_limited(ip_address):
    return limiter_storage.is_limited(ip_address)
...
def try_fold_string(node):
    if isinstance(node, ast.Constant) and isinstance(node.value, str):
        ...
        if isinstance(node.op, ast.Add):
            left_val = try_fold_string(node.left)
            right_val = try_fold_string(node.right)
            if left_val is not None and right_val is not None:
                return left_val + right_val
        elif isinstance(node.op, ast.Mult):
            left_val = try_fold_string(node.left)
            right_val = get_const_value(node.right)
        ...
        for val in node.values:
            if isinstance(val, ast.Constant) and isinstance(val.value, str):
                ...
                inner_val = try_fold_string(val.value)
        ...
        target_str = try_fold_string(node.value)
        if isinstance(target_str, str):
            if isinstance(node.slice, ast.Slice):
                step = None
                if node.slice.step is not None:
                    if isinstance(node.slice.step, ast.UnaryOp) and isinstance(node.slice.step.op, ast.USub):
                        op_val = get_const_value(node.slice.step.operand)
                        if isinstance(op_val, int):
                            step = -op_val
                    else:
                        step = get_const_value(node.slice.step)
                lower = get_const_value(node.slice.lower) if node.slice.lower is not None else None
                upper = get_const_value(node.slice.upper) if node.slice.upper is not None else None
                try:
        ...
        if len(node.generators) == 1:
            ...
            if isinstance(gen.target, ast.Name) and not gen.ifs:
                iter_val = try_fold_string(gen.iter)
                if isinstance(iter_val, str):
                    if isinstance(node.elt, ast.Name) and node.elt.id == gen.target.id:
                        return list(iter_val)
    elif isinstance(node, ast.Call):
        if isinstance(node.func, ast.Attribute):
            if node.func.attr == 'join':
                sep_str = try_fold_string(node.func.value)
                if isinstance(sep_str, str) and node.args:
                    list_val = try_fold_string(node.args[0])
                    if isinstance(list_val, list):
                        return sep_str.join(list_val)
                    list_node = node.args[0]
                    if isinstance(list_node, ast.List):
                        parts = []
                        valid = True
                        for el in list_node.elts:
                            part_val = try_fold_string(el)
            ...
            elif node.func.attr in ('lower', 'upper', 'strip'):
                target_str = try_fold_string(node.func.value)
                if isinstance(target_str, str):
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Name):
            if node.func.id in ('eval', 'exec'):
                ...
                if len(node.args) >= 2:
                    attr_name = try_fold_string(node.args[1])
                    if attr_name in ('eval', 'exec', 'system', 'popen'):
                        self.issues.append({
                            'file': self.filename,
                            'line': node.lineno,
                            'class': 'Obfuscated Execution Attempt',
                            'severity': 'HIGH',
                            'remediation': f"Dynamic extraction of unsafe attribute '{attr_name}' detected via getattr(). Refactor immediately."
                        })
...
def scan_regex_string(code_str, filename, regex_rules):
    ...
    inner_html_rx = re.compile(r"(?:\.innerHTML|\[\s*(?:['\"`]*innerHTML['\"`]*|['\"`]*inner['\"`]*\s*(?:\+|\$\{\s*['\"`]*)\s*['\"`]*HTML['\"`]*(?:\s*\})?)\s*\])\s*=")
...
class ScannerHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if is_rate_limited(client_ip):
            ...
            self.end_headers()
            self.wfile.write(json.dumps({'error': 'Too Many Requests (Limit: 60/min)'}).encode('utf-8'))
            return
